...
def historique_telecharger(request, id):
    ...
    try:
        if h.destination == 'google_drive':
            contenu = google_drive_service.telecharger_sauvegarde(h.google_drive_file_id)
        else:
...
def google_autoriser(request):
    ...
    un nouvel onglet — réservé au super admin. Le `state` généré est mémorisé
    côté serveur (cache, expire après 10 min) associé à l'admin à l'origine de
    la demande : pas de session Django dans ce projet (auth par token), donc
    rien d'autre ne permettrait de relier le callback Google (appelé
    directement par le navigateur, sans notre header Authorization) à un compte.
    """
    if request.method != 'GET':
        return JsonResponse({'error': 'Méthode non autorisée', 'error_code': 'METHOD_NOT_ALLOWED'}, status=405)
    ...
    try:
        url, state = google_drive_service.construire_url_autorisation()
    except google_drive_service.ErreurGoogleDrive as e:
        return JsonResponse({'error': str(e), 'error_code': 'GOOGLE_DRIVE_NOT_CONFIGURED'}, status=400)

    cache.set(f"gdrive_oauth_state:{state}", utilisateur.id, timeout=600)
...
def google_callback(request):
    ...
    admin_id = cache.get(cle_cache)
    if not admin_id:
        return redirect(cible + 'erreur')
    cache.delete(cle_cache)

    try:
        refresh_token = google_drive_service.echanger_code_contre_refresh_token(code)
    except google_drive_service.ErreurGoogleDrive:
        return redirect(cible + 'erreur')
