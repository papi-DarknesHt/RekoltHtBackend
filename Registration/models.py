...
class Utilisateur(models.Model):
    ...
    def __str__(self):
        nom_affiche = f"{self.prenom} {self.nom}".strip()
        return f"{nom_affiche} — {self.email}"
    ...
    def ajouter_avertissement(self):
        ...
        if self.nombre_avertissements >= SEUIL_AVERTISSEMENTS and not self.est_bloquer:
            self.bloquer(f"Suspension automatique après {self.nombre_avertissements} avertissements.")
            return True
        return False
...
class Profil(models.Model):
    ...
    def __str__(self):
        nom_affiche = f"{self.utilisateur.prenom} {self.utilisateur.nom}".strip()
        return f"Profil de {nom_affiche}"
...
class DemandeVerification(models.Model):
    ...
    STATUTS = [
        ...
        ('en_attente_manuelle', 'En attente de revue manuelle'),  # produit par _lancer_verification_faciale sur tout échec (infra ou score faible) — la vérification faciale automatique n'est pas jugée assez fiable pour rejeter seule, un admin tranche via traiter_demande_revue_manuelle
        ('verifie',             'Vérifié'),
        ('echoue',              'Échoué'),
    ]
    ...
    # le verso de la CIN a été retiré (demande explicite du propriétaire) :
    # jamais exploité par le pipeline OCR (_lancer_pipeline_ocr ne lit que
    # document_recto) — n'ajoutait qu'une étape de capture sans bénéfice réel
    ...
    motif_echec            = models.TextField(blank=True, null=True)  # rejet définitif (marquer_echoue)
    motif_revue_manuelle   = models.TextField(blank=True, null=True)  # raison de la mise en revue manuelle (marquer_en_attente_manuelle), distinct d'un rejet
    ...
    def marquer_verifie(self):
        ...
        # pour une entreprise, ce même succès doit aussi se refléter sur ses
        # propres champs de vérification (Entreprise.est_verifiee/
        # statut_verification, affichés sur sa page "Mon entreprise",
        # ProfilAcheteur.jsx) — sans ça la promotion au rôle vendeur passait
        # bien, mais la carte profil restait bloquée sur "en attente" pour
        # toujours (constaté lors d'un test bout en bout)
        if self.type_demandeur == 'entreprise':
            entreprise = Entreprise.objects.get(pk=self.utilisateur_id)
            entreprise.est_verifiee = True
            entreprise.statut_verification = 'valide'
            entreprise.save(update_fields=['est_verifiee', 'statut_verification'])
    ...
    def marquer_echoue(self, motif):
        ...
        # même raison que dans marquer_verifie() ci-dessus : refléter l'échec
        # sur Entreprise.statut_verification, pas seulement sur la demande
        if self.type_demandeur == 'entreprise':
            Entreprise.objects.filter(pk=self.utilisateur_id).update(statut_verification='rejete')
    ...
    def marquer_en_attente_manuelle(self, motif):
        """
        Route la demande vers une revue manuelle par un admin au lieu d'un
        échec/succès automatique — utilisé par _lancer_verification_faciale
        quand la vérification faciale DeepFace n'est pas jugée assez fiable
        pour décider seule (score sous le seuil OU erreur d'infrastructure
        VerificationFacialeIndisponible). Contrairement à marquer_echoue, ne
        rejette rien : un admin tranchera ensuite via marquer_verifie()/
        marquer_echoue() (voir Registration/views.py::traiter_demande_revue_manuelle).
        """
        self.statut = 'en_attente_manuelle'
        self.motif_revue_manuelle = motif
        self.date_traitement = timezone.now()
        self.save()
        ...
        try:
            send_mail(
                subject        = "Votre vérification RekoltHt est en cours de revue",
                message        = (
                    f"Bonjour {nom_complet},\n\n"
                    "Votre demande de vérification nécessite une revue manuelle "
                    "par notre équipe. Vous serez notifié dès qu'une décision "
                    "sera prise.\n\n"
                    "L'équipe RekoltHt"
                ),
                from_email     = settings.DEFAULT_FROM_EMAIL,
...
class InscriptionEnAttente(models.Model):
    ...
    Couvre aussi l'inscription entreprise (voir creerEntreprise) — même
    exigence explicite : le mail de l'entreprise doit être opérationnel avant
    que son compte n'existe. type_compte distingue les deux cas ; pour une
    entreprise, nom porte nom_Entreprise et prenom reste vide (l'entreprise
    n'a pas de prénom), les champs propres à l'entreprise (secteur, adresse,
    logo...) vivent dans donnees_optionnelles comme pour un compte individuel.
    """
    ...
    type_compte  = models.CharField(max_length=20, choices=TYPE_COMPTE, default='individuel')
    email        = models.EmailField(unique=True)   # une nouvelle demande pour le même email remplace l'ancienne
    nom          = models.CharField(max_length=100)   # nom_Entreprise si type_compte='entreprise'
    prenom       = models.CharField(max_length=100, blank=True)   # toujours vide pour une entreprise
    mot_de_passe = models.CharField(max_length=255)  # déjà haser_password(...), jamais en clair
    telephone    = models.CharField(max_length=20)
    # champs optionnels du formulaire — individuel : bio, photo_profil,
    # adresse, commune, ville, pays, role, latitude, longitude ; entreprise :
    # secteur, description, adresse, departement, commune, section_communale,
    # pays, longitude, latitude, logo — ré-appliqués à la confirmation, voir
...
def creer_ou_obtenir_compte_proprietaire():
    """
    Garantit l'existence du compte propriétaire de la plateforme
    (rekoltht@gmail.com), avec tous les droits pratiques (super_admin) et,
    sauf état anormal préexistant (voir plus bas), est_super_super_admin —
    quel que soit l'état du reste de la base. Contrairement à l'ancien
    amorçage post_migrate (voir creer_compte_proprietaire_si_base_vide,
    Registration/signals.py), qui ne s'exécute que sur une base entièrement
    vide, cette fonction est appelée à la fois au démarrage (base vide) ET
    depuis seConnecter (Registration/views.py) dès qu'une tentative de
    connexion cible cet email et qu'il est absent — pour qu'il soit
    toujours récupérable, même si d'autres comptes existent déjà en base
    (incident réel : le compte avait disparu alors que d'autres comptes
    existaient, et ne pouvait donc jamais être recréé automatiquement par
    l'ancien mécanisme, réservé au tout premier compte de la plateforme).

    Idempotente : si le compte existe déjà, le retourne tel quel sans rien
    modifier (aucun risque d'écraser un mot de passe déjà changé).

    Mot de passe volontairement faible ("1234", demande explicite du
    propriétaire) si (re)créé : doit_changer_mot_de_passe=True force son
    changement dès la connexion suivante.
    """
    proprietaire = Utilisateur.objects.filter(email="rekoltht@gmail.com").first()
    if proprietaire is not None:
        return proprietaire

    proprietaire = Utilisateur.objects.create(
        nom="RekoltHT", prenom="", email="rekoltht@gmail.com",
        mot_de_passe=haser_password("1234"), telephone="",
        est_actif=True, doit_changer_mot_de_passe=True,
    )
    # creer_profil (signal post_save, voir Registration/signals.py) crée déjà
    # son Profil, et ne lui accorde admin + est_super_super_admin QUE si
    # c'est le tout premier compte de la plateforme — pas garanti ici.
    # Accordé donc explicitement ci-dessous. est_super_super_admin ne l'est
    # que si aucun autre compte ne le détient déjà (jamais dupliqué, voir
    # peut_agir_sur_admin) ; dans le cas anormal contraire (un autre compte
    # a hérité du statut propriétaire entretemps — ne devrait jamais arriver
    # en usage normal), ce compte reçoit quand même tous les droits
    # pratiques (super_admin) plutôt que d'être laissé sans aucun droit.
    proprietaire.profil.role = 'admin'
    proprietaire.profil.save(update_fields=['role'])
    # exclut le compte qu'on est en train de traiter : creer_profil
    # (ci-dessus) peut déjà lui avoir créé sa propre ligne DroitsAdmin avec
    # est_super_super_admin=True s'il se trouve être le tout premier compte
    # de la plateforme — sans cette exclusion, il se détecterait lui-même
    # comme "un autre propriétaire déjà existant" et s'auto-rétrograderait
    # (bug constaté : reproduit avec un flush complet de la base, le seul
    # compte restant après coup n'obtenait plus est_super_super_admin).
    est_seul_proprietaire = not DroitsAdmin.objects.filter(est_super_super_admin=True).exclude(utilisateur=proprietaire).exists()
    DroitsAdmin.objects.update_or_create(
        utilisateur=proprietaire,
        defaults=dict(
            super_admin=True, est_super_super_admin=est_seul_proprietaire,
            gestion_utilisateurs=True, gestion_signalements=True,
            gestion_categories=True, gestion_support=True,
            gestion_sauvegardes=True, gestion_mots_de_passe=True,
        ),
    )
    return proprietaire
...
class DemandeAdministrative(models.Model):
    ...
    def approuver(self, admin, reponse=''):
        ...
        # Registration/models.py, classe Entreprise) — repli sur .nom
        # (nom_Entreprise) pour ne pas envoyer "Bonjour ,"
        prenom = (self.utilisateur.prenom or self.utilisateur.nom) if self.utilisateur_id else (self.nom_contact.split(' ')[0] if self.nom_contact else '')
        self._notifier(
...
class CompteSupprime(models.Model):
    ...
    def __str__(self):
        nom_affiche = f"{self.prenom} {self.nom}".strip()
        return f"Compte supprimé — {nom_affiche} ({self.email})"
