...
def modifierProduit(request):
    ...
    elif 'categorie_id' in data and not sousCategories.objects.filter(id=produit.sous_categorie_id, categorie=produit.categorie).exists():
        # categorie_id changée seule, sans nouvelle sous_categorie_id : la
        # sous-catégorie déjà enregistrée appartenait à l'ancienne catégorie
        # et deviendrait incohérente avec la nouvelle (désynchronisation
        # catégorie/sous-catégorie) — exiger explicitement une nouvelle
        # sous-catégorie plutôt que persister cet état invalide
        return JsonResponse({
            'error': "La sous-catégorie actuelle du produit ne correspond pas à la nouvelle catégorie ; "
                     "veuillez fournir sous_categorie_id.",
            'error_code': 'SOUS_CATEGORIE_INCOHERENTE',
        }, status=400)
...
# ── VENDEURS — CAROUSEL D'ACCUEIL (public) ────────────────────────────────────
@csrf_exempt
def listerVendeursPublics(request):
    """
    TOUS les comptes vendeurs (profil individuel ou entreprise), qu'ils aient
    ou non un produit disponible en ce moment — alimente le carousel "Nos
    vendeurs" de la page d'accueil (voir HomePage.jsx). Contrairement à
    listerVendeursCarte ci-dessus, AUCUNE position GPS n'est requise : ce
    carousel n'affiche pas de carte, la position ne serait qu'une contrainte
    inutile qui exclurait des vendeurs valides.
    """
    if request.method != 'GET':
        return JsonResponse({'error': 'Méthode non autorisée', 'error_code': 'METHOD_NOT_ALLOWED'}, status=405)
    ...
    vendeurs = []
    for vendeur in Utilisateur.objects.filter(profil__role='vendeur').select_related('profil'):
        entreprise = Entreprise.objects.filter(pk=vendeur.id).first()
        source = entreprise if entreprise else vendeur.profil
